...
`)
	...
	t.Run("decorated function", func(t *testing.T) {
		...
		require.NoError(t, err)
		function := mod.Body[0].(*ast.Function)
		require.Equal(t, "staticmethod", function.Decorators[0].(*ast.Name).Name)
...
`)
		...
		require.Equal(t, "Base", cls.Bases[0].(*ast.Name).Name)
		require.Equal(t, "dataclass", cls.Decorators[0].(*ast.Name).Name)
...
`)
	...
	t.Run("class decorator semantics are deferred to the checker", func(t *testing.T) {
		...
		require.NoError(t, err)
		cls := mod.Body[0].(*ast.Class)
		require.Equal(t, "other", cls.Decorators[0].(*ast.Name).Name)
	})

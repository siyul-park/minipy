...
func (c *checker) methodCallType(n *ast.CallExpr, attr *ast.Attribute) types.Type {
	...
	// A receiver with no method catalogue and no arm above still has its
	// arguments checked, so one pass reports every diagnostic inside them
	// alongside the unsupported method itself.
	c.positionalMethodArgs(n)
	c.errs.Add(n.Pos(), token.UnsupportedFeature, "method %s on %s is not supported", attr.Name, receiver)
	return types.Invalid
}
